...
* Relying on LXC posed several problems for the Docker projects
    ...
    * To improve the experience and help the project evolved more quickly, Docker replaced LXC with its own tool, `libcontainer`, whose goal was to be a platform-agnostic tool that gave Docker access to the fundamental container building block in the host kernel

* The Docker engine was originally a monolithic, with almost all funcionality coded into the daemon. However, as time passed, this became more and more problematic.
...
* They have even added a third specification:
...
* `containerd` is refered as a high-level runtime as it manages lifecycle events such as starting, stopping, and deleting containers.
* It needs a low-level runtime to perform the actual work. Most of the time containerd is paired with runc as its low-level runtime
...
1. When we run commands like this, the Docker client converts them into API requests and send them to the API exposed by the daemon. 
2. The daemon receives the request, interprets it as a request to create a new container, and passed it to containerd. the daemon communicates with containerd via a CRUD-style API. 
3. containerd converts the requiered Docker image into an OCI bundle and tells runc to use this to create a new container. 
4. runc interfaces with the OS kernel to pull together all the constructs necessary to create a container (namespaces, cgroups, etc). The container starts as a child process of runc, and as soon as the container start, runc exits
